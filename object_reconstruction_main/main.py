...
from manage_visuals import ManageVisuals
import rospy
import numpy as np
...
class MainClass():
    def __init__(self, params, root_path):
        self.pr = PyRep()
        self.params = params
    ...
    def move_to_NBV_no_path(self, nbv_pose_dq, circ_dir=1):
        ...
        iteration_counter = 0
        min_iterations_for_curvature = 30
        robot_path_list = []
        ...
        while task_err_norm > self.params["ControllerSettings"]["errorTolerance"]:
            ...
            if self.params["ControllerSettings"]["includeCurvatureCheck"]:
                ...
                if transition_err_norm > 0.2:
                    if iteration_counter > min_iterations_for_curvature:
                        diffs = np.diff(np.array(robot_path_list), axis=0)
                        angles = np.arctan2(diffs[:, 1], diffs[:, 0])
                        ang_diffs = np.abs(np.diff(angles))
                        # Handle angle wrapping
                        ang_diffs = np.minimum(ang_diffs, 2*np.pi - ang_diffs)
                        total_angle = np.sum(ang_diffs)
                        curvature = total_angle / \
                            (np.linalg.norm(diffs, axis=1).sum()+1e-6)
        ...
        self.pr.step()
        termination_reason = ""
        if not is_high_curvature and not is_stability_error:
            termination_reason = TerminationReason.SUCCESS
        elif is_high_curvature:
    ...
    def move_to_NBV_focus_point(self, nbv_pose_dq, nbv_initial_q, circ_dir=1):
        ...
        while task_err_norm > self.params["ControllerSettings"]["errorTolerance"]:
            ...
            prev_task_err_norm = task_err_norm
        # The robot has reached the NBV or terminated due to high curvature or stability error
        # Stop the robot.
        self.robot_model.send_velocities(np.zeros(8))
        self.pr.step()
        termination_reason = ""
    ...
    def move_to_NBV_sampling(self, nbv_pose_dq, circ_dir=1, show_rrt_paths=False):
        ...
        for node_idx in range(len(rrt_path_np)+1):
                ...
                if self.params["ControllerSettings"]["includeCurvatureCheck"]:
                    ...
                    if transition_err_norm > 0.2:
                        if iteration_counter > min_iterations_for_curvature:
                            ...
                            ang_diffs = np.abs(np.diff(angles))
                            # Handle angle wrapping
                            ang_diffs = np.minimum(
                                ang_diffs, 2*np.pi - ang_diffs)
                            total_angle = np.sum(ang_diffs)
                            curvature = total_angle / \
                                (np.linalg.norm(diffs, axis=1).sum()+1e-6)
            ...
            curvature_for_all_views.append(curvature)
        ...
        result_dic = {"iteration_counter": int(np.array(iteration_count_for_all_views).sum()),
                      ...
                      "details_for_views": {
                          ...
                          "curvatures": curvature_for_all_views,
...
if __name__ == "__main__":
    root_path = "/home/fth/focus_organized/src/"
    rospy.init_node("object_reconstruction_coppelia_node", anonymous=True)
    with open(root_path+"object_reconstruction_main/Config/config.yaml", 'r') as configFile:
    ...
    try:
        nbv_obj = MainClass(configs, root_path)
